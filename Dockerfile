...
RUN apt-get update && apt-get install -y libfontconfig1-dev libgraphite2-dev libharfbuzz-dev libicu-dev zlib1g-dev fonts-ipafont
RUN cargo install tectonic --force --vers 0.2.0
...
RUN apt-get update \
    && apt-get install -y --no-install-recommends libfontconfig1 libgraphite2-3 libharfbuzz0b zlib1g libharfbuzz-icu0 libssl1.1 ca-certificates curl jq fonts-ipafont
